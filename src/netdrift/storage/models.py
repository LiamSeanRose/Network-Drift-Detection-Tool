...
class DriftExplanation(Base):
    ...
    One row per unique drift fingerprint — the same v2.0 key KnownIssue and
    Acknowledgement use — so an explanation is generated once per *structural*
    drift, not once per drift_events row. Populated on the scheduler cycle by
    explain.explain() when an LLM provider is configured; absent when the AI
    feature is off, in which case the API falls back to the deterministic
    diagnose.py causes.
    """

    __tablename__ = "drift_explanations"

    fingerprint: Mapped[str] = mapped_column(String, primary_key=True)
    explanation: Mapped[str] = mapped_column(String)
    # "llm" when generated by a model, "deterministic" when the model was
    # unavailable and the diagnose.py fallback was stored instead.
    source: Mapped[str] = mapped_column(String)
    generated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True))

    def __repr__(self):
        return (
            f"<DriftExplanation fingerprint={self.fingerprint!r} "
